...
function ink(ctx:CanvasRenderingContext2D,value:string,x:number,y:number){ctx.save();ctx.strokeStyle='#000';ctx.lineWidth=TEXT_STROKE;ctx.lineJoin='round';ctx.strokeText(value,x,y);ctx.restore();ctx.fillText(value,x,y)}
function text(ctx:CanvasRenderingContext2D,value:string,x:number,y:number,maxWidth:number,size:number,weight=400){let px=size;while(px>18){ctx.font=`${weight} ${px}px Arial`;if(ctx.measureText(value).width<=maxWidth)break;px--}ink(ctx,value,x,y)}
...
async function renderLayer(type:PassType,e:EmployeeData,layer:Layer){const{canvas,ctx}=await base(layer);
 if(type==='temporary'){
  if(layer!=='black'){const color=await load(asset('emblem-color.png'));contain(ctx,color,32,96,375,505)}if(layer!=='color'){ctx.font='400 72px Arial';ink(ctx,'ВРЕМЕННЫЙ',466,216);ctx.font='400 104px Arial';ink(ctx,'ПРОПУСК',466,392);ctx.font='400 37px Arial';ink(ctx,'№ Пропуска',466,548);ctx.font='400 39px Arial';ink(ctx,e.passNumber||'',466,611);ctx.font='700 39px Arial';ink(ctx,'МО',914,609)}return canvas.toDataURL('image/png')
 }
 ...
 const fit=normalizePosition(ctx,e.position||'',{fontFamily:'Arial',fontSize:40,minScale:.72,maxWidth:w,maxLines:2,lineHeight:44},true);ctx.font=`400 ${fit.fontSize}px Arial`;fit.lines.forEach((line,index)=>ink(ctx,line,x,324+index*44));
 if(type==='mosn'){ctx.font='400 46px Arial';ink(ctx,'МОСН',x,437);ink(ctx,'№ Пропуска',x,546);ink(ctx,e.passNumber||'',x,608)}else{ctx.font='400 46px Arial';const tabLabel='Таб. №';ink(ctx,tabLabel,x,429);const tabX=x+ctx.measureText(tabLabel).width+10;ink(ctx,e.employeeNumber||'',tabX,429);ink(ctx,'№ Пропуска',x,528);ink(ctx,e.passNumber||'',x,590)}
