...
function emblem(ctx:CanvasRenderingContext2D,image:HTMLImageElement,x:number,y:number,w:number,h:number){ctx.drawImage(image,x,y,w,h)}

export async function renderCard(type:PassType,e:EmployeeData){const{canvas,ctx}=await base();
 if(type==='temporary'){
  const color=await load(asset('emblem-color.png'));emblem(ctx,color,32,96,375,505);ctx.font='400 58px Arial';ctx.fillText('ВРЕМЕННЫЙ',466,216);ctx.font='400 76px Arial';ctx.fillText('ПРОПУСК',466,392);ctx.font='400 37px Arial';ctx.fillText('№ Пропуска',466,548);ctx.font='400 39px Arial';ctx.fillText(e.passNumber||e.employeeNumber||'',466,611);ctx.font='700 39px Arial';ctx.fillText('МО',914,609);return canvas.toDataURL('image/png')
 }
 if(e.photo?.dataUrl){try{photoFrame(ctx,await load(e.photo.dataUrl))}catch{}}const black=await load(asset('emblem-black.png'));emblem(ctx,black,800,425,205,205);
 ...
 if(type==='mosn'){ctx.font='400 44px Arial';ctx.fillText('МОСН',x,444);ctx.font='400 35px Arial';ctx.fillText('№ Пропуска',x,552);ctx.font='400 39px Arial';ctx.fillText(e.passNumber||'',x,617)}else{ctx.font='400 35px Arial';ctx.fillText('Таб. №',x,431);ctx.fillText(e.employeeNumber||'',620,431);ctx.fillText('№ Пропуска',x,530);ctx.font='400 39px Arial';ctx.fillText(e.passNumber||'',x,599)}
