...
async function base(layer:Layer){const canvas=document.createElement('canvas');canvas.width=CARD.widthPx;canvas.height=CARD.heightPx;const ctx=canvas.getContext('2d')!;ctx.fillStyle='#fff';ctx.fillRect(0,0,1012,638);if(layer!=='black'){const background=await load(asset('medical-background.jpg'));ctx.drawImage(background,0,64,440,574)}ctx.fillStyle='#111';ctx.textBaseline='alphabetic';ctx.textAlign='left';if(layer!=='color'){ctx.font='400 43px Arial';ctx.textAlign='center';ctx.fillText(ORGANIZATION,506,50);ctx.textAlign='left'}return{canvas,ctx}}
